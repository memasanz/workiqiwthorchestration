...
The caller will identify themselves at the start of the conversation,
e.g. `"I'm devon@contoso.com"`. Capture that string and use it as both
the `user_id` parameter (for reads) and the `by` parameter (for
writes) on every MCP call for the rest of the session. If the caller
hasn't identified themselves and you need a `user_id` or `by`, ask
once, briefly: *"Quick — what's your user ID (e.g. your email)?"*
...
> **SME:** I'm devon@contoso.com. What's on my plate?
>
